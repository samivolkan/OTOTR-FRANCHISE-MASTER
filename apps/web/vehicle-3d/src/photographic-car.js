...
export class PhotographicCar{
  ...
  setSource({photo,mask}){
    ...
    const width=mask.width||1072,height=mask.height||586;this.sourceWidth=width;this.sourceHeight=height;
    const stage=svg('svg',{viewBox:`${-width*.126} ${-height*.191} ${width*1.252} ${height*1.382}`,preserveAspectRatio:'xMidYMid meet',class:'photographic-svg','aria-label':'Gerçek fotoğrafın görünür parçaları'});
    const defs=svg('defs'),baseMask=svg('mask',{id:this.prefix+'-base',maskUnits:'userSpaceOnUse',x:0,y:0,width,height});
    baseMask.append(svg('rect',{width,height,fill:'white'}));
    ...
    this.context=svg('image',{href:photo.url,width,height,mask:'url(#'+this.prefix+'-base)','pointer-events':'none'});
    ...
    for(const p of mask.parts){
      ...
      const image=svg('image',{href:photo.url,width,height,'clip-path':'url(#'+this.prefix+'-'+p.id+')','pointer-events':'none'});
  ...
  render(){
    ...
    for(const layer of this.layers){const{part:p,group,edge,line}=layer,isSelected=p.id===this.selected;
      ...
      if(isolating&&isSelected){const box=edge.getBBox();scale=clamp(Math.min(this.sourceWidth*.578/Math.max(box.width,1),this.sourceHeight*.623/Math.max(box.height,1)),1.25,3.4);dx=this.sourceWidth*.5-p.anchor[0];dy=this.sourceHeight*.507-p.anchor[1];}
