...
async function main(){
 ...
 try{
  ...
  assert.equal(new URL(spin.url()).pathname,'/Ototr/kaporta-360/');assert.equal(await spin.title(),'Kaporta 360 | OTOTR ERP');checks.push('ERP menu opens Kaporta 360 at its published subpath');
  ...
  assert.equal(await spin.locator('a[href="./index.html"]').count(),0);assert.equal(await spin.locator('.r-header nav a').getAttribute('href'),'../index.html#dashboard');checks.push('ERP branding, return link and demo/source labels');
  ...
  await spin.locator('.r-header nav a').click();await spin.locator('#nav [data-nav-route="kaporta-360"]').waitFor();assert.equal(new URL(spin.url()).hash,'#dashboard');checks.push('return to ERP dashboard');
