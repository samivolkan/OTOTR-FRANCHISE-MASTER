...
fs.writeFileSync(path.join(output,'release.json'),JSON.stringify({module:'kaporta-360',buildTime:new Date().toISOString(),frames:24,parts:23,mode:'demo',inspectionDataIsFictional:true,geometryReconstructed:false},null,2));
console.log('ERP paketi hazır: dist-erp/ — 24 doğrulanmış fotoğraf, 23 parça, yalnız gerekli yayın dosyaları.');
