...
const expectedNavLabels = ["Görevler", "İşlerim", "Ana Sayfa", "Eksikler", "Profil"];
const expectedNavRoutes = ["tests", "jobs", "home", "missing", "profile"];
assert(
  JSON.stringify(navigationItems.map((item) => item.label)) === JSON.stringify(expectedNavLabels),
...
assert(expertiseSchemaStats.sourceFound === true, "JSON şeması okunamadı.");
  ...
  for (const item of form.items.slice(0, 20)) {
    for (const option of item.options) {
      if (option.needsLabelMap) {
        assert(option.displayLabel === "Seçenek etiketi doğrulanacak", `${key} içinde ham checkbox ID gösterimi sızdı.`);
...
for (const status of ["Orijinal", "Boyalı", "Lokal Boyalı", "Değişen", "Hasarlı", "Kontrol Edilmedi"]) {
  assert(bodyPanels.some((panel) => panel.status === status), `Panel durumu eksik: ${status}`);
}
...
assert(unresolvedSelectedOptions.length >= 1, "Çözümsüz seçilmiş option uyarısı bekleniyor.");
assert(finalApprovalGate.fallbackLabel === "Seçenek etiketi doğrulanacak", `Fallback label hatalı: ${finalApprovalGate.fallbackLabel}`);
assert(optionLabelFallback === "Seçenek etiketi doğrulanacak", `optionLabelFallback hatalı: ${optionLabelFallback}`);
...
assert(
  finalApprovalGate.blockingMessage === "Bu seçeneklerin açıklaması doğrulanmadığı için iş emri tamamlanamaz.",
...
assert(finalApprovalWarnings.some((item) => item.title === "Doğrulama Bekleyen Seçenek"), "Final onay uyarılarında doğrulama bekleyen seçenek eksik.");
assert(reportBlockedData.blockers.some((item) => item.title === "Doğrulama Bekleyen Seçenek"), "Rapor bloklama ekranında doğrulama bekleyen seçenek eksik.");
assert(technicalApprovalDetail.unresolvedWarnings.length >= 1, "Teknik onay detayında çözümsüz option uyarısı eksik.");
assert(revisionRequestData.unresolvedWarnings.length >= 1, "Düzeltme istendi ekranında çözümsüz option uyarısı eksik.");
...
assert(reportApprovedData.statusTitle === "İş Emri Tamamlandı", "Tamamlanan rapor ekranı iş emri tamamlandı terminolojisine bağlı değil.");
assert(reportApprovedData.completionLabel === "Rapor Tamamlandı", "Tamamlanan rapor ekranı rapor tamamlandı metnini taşımıyor.");
assert(technicalApprovalTerminology.technicianFinalAction === "İş Emrini Tamamla", "Teknisyen final aksiyonu iş emrini tamamla olmalı.");
...
for (const section of ["Müşteri Özeti", "İç Teknik Not", "Kritik Bulgular", "İyi Durumlar", "Fotoğraf Sayısı", "Önizle / Düzenle"]) {
  assert(customerSummarySections.includes(section), `Müşteri özeti/final rapor görünür bölümü eksik: ${section}`);
}
...
for (const label of [
  ...
  assert(fieldValidationHints.some((item) => item.label === label), `FAZ 5.5 alan doğrulama notu eksik: ${label}`);
}
...
for (const mojibake of ["Ã", "Â", "Å", "Ä", "â€", "â€¢"]) {
  if (runtimeText.includes(mojibake)) errors.push(`Bozuk Türkçe karakter izi bulundu: ${mojibake}`);
}

const auditData = readJson(resolve(appRoot, "src/data/expertise/unknown-option-label-audit.json"));
assert(auditData?.fallbackLabel === "Seçenek etiketi doğrulanacak", "unknown-option-label-audit fallback label hatalı.");
...
assert(phase2ScreenSource.includes("app-header jobs-approved-header"), "Islerim header app-header sinifina baglanmadi.");
...
assert(phase2ScreenSource.includes("home-standard-main"), "Ana Sayfa Islerim kokpit standardina bagli degil.");
assert(phase2ScreenSource.includes("homeStandardOrderSection"), "Ana Sayfa is kartlari ortak section helper'i ile uretilmeli.");
assert(phase2ScreenSource.includes("homeStandardKpis"), "Ana Sayfa KPI'lari Islerim KPI ailesiyle uretilmeli.");
assert(phase2ScreenSource.includes("jobsApprovedCard(order, onNavigate)"), "Ana Sayfa is kartlari Islerim kart component'ini kullanmali.");
...
assert(!phase2ScreenSource.includes("selectedValue"), "UI renderer çözümsüz option selectedValue alanını kullanmamalı.");
...
assert(!runtimeText.includes(forbiddenOldCenterFab), "Eski bagimsiz kirmizi merkez varyasyonu kaldirilmali.");
...
assert(runtimeText.includes("photoSlotGrid"), "FAZ 7 fotoğraf/kanıt slot render helperı eksik.");
assert(runtimeText.includes("inputGrid"), "FAZ 7 input/ölçüm alanı render helperı eksik.");
...
assert(runtimeText.includes("İş Emrini Tamamla"), "İş Emrini Tamamla mantığı korunmuyor.");
assert(runtimeText.includes("Seçenek etiketi doğrulanacak"), "Label fallback metni korunmuyor.");
assert(runtimeText.includes("Fotoğraf ekleme bu fazda opsiyoneldir."), "Opsiyonel fotoğraf uyarısı eksik.");
...
assert(runtimeText.includes("İşe Başla"), "Ana Sayfa Hızlı İşlemler içinde işe başlama aksiyonu korunmalı.");
...
assert(runtimeText.includes("jobs-approved-header"), "Onaylı İşlerim avatar başlığı eksik.");
...
assert(!phase2ScreenSource.includes("html: `<h3>${order.plaka}</h3><strong>${order.işEmriNo}</strong>"), "İşlerim liste kartında iş emri numarası görünür durumda.");
...
assert(phase2ScreenSource.includes('["Tümü", "Devam Eden", "Bekleyen", "Tamamlanan", "Eksik"]'), "Islerim tablari referans tasarimdaki sade 5 secenekten olusmali.");
assert(!/Devam Eden\s+\d|Bekleyen\s+\d|Tamamlanan\s+\d|Eksik\s+\d/.test(phase2ScreenSource), "Islerim tablarinda mukerrer sayac/rozet olmamali.");
...
for (const status of ["Bekliyor", "Devam Ediyor", "Eksik / Uyarı", "Tamamlanmaya Hazır"]) {
  assert(workOrders.some((order) => order.durum === status), `Mock iş emri durumu eksik: ${status}`);
}
...
for (const order of workOrders) {
  for (const field of ["işEmriNo", "plaka", "marka", "model", "yıl", "paket", "bayi", "müşteriAdı", "durum", "öncelik", "atanmışUsta", "başlamaDurumu", "teknikOnayDurumu"]) {
    assert(Object.hasOwn(order, field), `Mock iş emri alanı eksik: ${field}`);
  }
}

assert(technicalApprovalFlow.technicianCanApprove === false, "Teknisyen rapor onaylayabilir görünüyor.");
assert(technicalApprovalFlow.technicianFinalAction === "İş Emrini Tamamla", "Teknisyen final aksiyonu iş emrini tamamla olmalı.");

if (errors.length > 0) {
  console.error("FAZ 4 doğrulaması başarısız:");
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

console.log("Alt navigasyon dogrulamasi gecti: Gorevler / Islerim / OtoTR / Eksikler / Profil ve kirmizi Home FAB standart.");
