-- OTOTR reviewed migration 002: report template and report answer layer.
-- Source: packages/database/raw-migrations/schema-foundations/2026-05-25-report-template-system.sql
-- Target: local Supabase/Postgres validation first.
--
-- Review adjustments:
-- - Creates app_private schema before private helper use.
-- - Adds public current_app_user_id/current_app_user_role helpers when absent.
-- - Uses public.current_user_can_access_branch from reviewed baseline 001.
-- - Drops triggers before recreating them for idempotency.
-- - Keeps explicit grants for authenticated Data API access.
--
-- Do not apply to staging or production until local reset, RLS verification,
-- and app contract checks pass.
...
drop trigger if exists work_order_report_answers_updated_at on public.work_order_report_answers;
