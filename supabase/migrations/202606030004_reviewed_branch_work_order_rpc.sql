-- OTOTR reviewed migration 004: branch work order RPC layer.
-- Source: packages/database/raw-migrations/rpc-functions/2026-05-25-branch-work-order-rpc.sql
-- Target: local Supabase/Postgres validation first.
--
-- Review adjustments:
-- - Extends task status constraint for current app vocabulary.
-- - Uses public.current_user_can_access_branch from reviewed baseline 001.
-- - Removes RECEPTION_STAFF from role checks because baseline app_users role constraint does not allow it.
--
...
begin
  ...
  if not public.current_user_can_access_branch(current_task.branch_id) then
