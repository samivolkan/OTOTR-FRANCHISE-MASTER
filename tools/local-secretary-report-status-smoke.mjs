...
async function main() {
  ...
  console.log("PASS manager login for secretary report-status fixture setup");
  console.log("PASS technician login for final report status RPC");
  ...
  const caseId = await authedPost(status, managerToken, "/rpc/create_branch_work_order", {
    customer_full_name: "Sekreterya Rapor Smoke Musteri",
    customer_phone: "+905550008899",
    customer_email: "secretary-report-smoke@example.test",
    customer_identity_number: "",
    customer_role: "OWNER",
    vehicle_plate: `16 SEC ${suffix.slice(-3)}`,
    vehicle_vin: `SECREPORT${suffix}`.slice(0, 17),
    vehicle_brand: "BMW",
    ...
    vehicle_arrival_note: "Local secretary report-status smoke",
    package_type: "STANDARD",
    work_order_notes: "Sekreterya final rapor durum smoke"
  });
  console.log(`PASS branch work order created: ${caseId}`);

  const beforeRows = await authedPost(status, managerToken, "/rpc/list_branch_work_orders_with_report_status", {
    limit_count: 25
  });
  const before = findCase(beforeRows, caseId);
  if (!before || before.final_report_id !== null || before.final_report_status !== null) {
    throw new Error(`Expected new work order without final report, got ${JSON.stringify(before).slice(0, 260)}`);
  }
  console.log("PASS secretary list shows new work order before mobile final report");

  await authedPost(status, technicianToken, "/rpc/transition_mobile_work_order_status", {
    target_case_id: caseId,
    next_status: "IN_PROGRESS",
    transition_reason: "Secretary status smoke start proof"
  });
  ...
  if (!motorTask?.id) {
    throw new Error(`Expected motor task for secretary status smoke, got ${JSON.stringify(motorTaskRows).slice(0, 240)}`);
  }
  ...
  if (!claimedTask?.id || claimedTask.owner_user_id == null) {
    throw new Error(`Expected technician claim for secretary status smoke, got ${JSON.stringify(claimedTask).slice(0, 240)}`);
  }
  ...
  const answer = await authedPost(status, technicianToken, "/rpc/save_mobile_inspection_item_value", {
    ...
    target_description_text: "Riskli bulgu, kanit eklendi",
  ...
  await authedPost(status, technicianToken, "/rpc/register_inspection_evidence_upload", {
    ...
    metadata: { source: "local-secretary-report-status-smoke" }
  });
  ...
  const afterRows = await authedPost(status, managerToken, "/rpc/list_branch_work_orders_with_report_status", {
    limit_count: 25
  });
  const after = findCase(afterRows, caseId);
